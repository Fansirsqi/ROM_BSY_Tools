...
class RangeSet(object):
    ...
    def union(self, other):
        ...
        out = []
        z = 0
        for p, d in heapq.merge(zip(self.data, itertools.cycle((+1, -1))), zip(other.data, itertools.cycle((+1, -1)))):
    ...
    def overlaps(self, other):
        ...
        z = 0
        for _, d in heapq.merge(zip(self.data, itertools.cycle((+1, -1))), zip(other.data, itertools.cycle((+1, -1)))):
            if (z == 1 and d == 1) or (z == 2 and d == -1):
                return True
            z += d
        return False
    ...
    def map_within(self, other):
        ...
        offset = 0
        start = None
        for p, d in heapq.merge(zip(self.data, itertools.cycle((-5, +5))), zip(other.data, itertools.cycle((-1, +1)))):
            if d == -5:
                start = p
            elif d == +5:
                offset += p - start
