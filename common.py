...
def LoadInfoDict(input_file, input_dir=None):
    ...
    if input_dir is not None:
        ...
        if d.get('system_root_image') == 'true':
            ...
            d['ramdisk_fs_config'] = os.path.join(input_dir, 'META', 'root_filesystem_config.txt')
        ...
        if 'system_base_fs_file' in d:
            ...
            if os.path.exists(system_base_fs_file):
                ...
                print('Warning: failed to find system base fs file: %s' % (system_base_fs_file,))
        ...
        if 'vendor_base_fs_file' in d:
            ...
            if os.path.exists(vendor_base_fs_file):
                ...
                print('Warning: failed to find vendor base fs file: %s' % (vendor_base_fs_file,))
...
def LoadRecoveryFSTab(read_helper, fstab_version, system_root_image=False):
    ...
    if fstab_version == 1:
        ...
        for line in data.split('\n'):
            ...
            d[mount_point] = Partition(mount_point=mount_point, fs_type=pieces[1], device=pieces[2], length=length, device2=device2)
        ...
        for line in data.split('\n'):
            ...
            d[mount_point] = Partition(mount_point=mount_point, fs_type=pieces[2], device=pieces[0], length=length, device2=None, context=context)
...
def _BuildBootableImage(sourcedir, fs_config_file, info_dict=None, has_ramdisk=False):
    ...
    p.communicate()
    assert p.returncode == 0, 'mkbootimg of %s image failed' % (os.path.basename(sourcedir),)

    if info_dict.get('boot_signer', None) == 'true' and info_dict.get('verity_key', None):
        path = '/' + os.path.basename(sourcedir).lower()
        cmd = [OPTIONS.boot_signer_path]
        cmd.extend(OPTIONS.boot_signer_args)
        cmd.extend([path, img.name, info_dict['verity_key'] + '.pk8', info_dict['verity_key'] + '.x509.pem', img.name])
        p = Run(cmd, stdout=subprocess.PIPE)
        p.communicate()
    ...
    # Sign the image if vboot is non-empty.
        ...
        cmd = [info_dict['vboot_signer_cmd'], info_dict['futility'], img_unsigned.name, info_dict['vboot_key'] + '.vbpubk', info_dict['vboot_key'] + '.vbprivk', info_dict['vboot_subkey'] + '.vbprivk', img_keyblock.name, img.name]
        p = Run(cmd, stdout=subprocess.PIPE)
        p.communicate()
...
def GetBootableImage(name, prebuilt_name, unpack_dir, tree_subdir, info_dict=None):
    ...
    # for recovery.
    has_ramdisk = info_dict.get('system_root_image') != 'true' or prebuilt_name != 'boot.img' or info_dict.get('recovery_as_boot') == 'true'
...
def UnzipTemp(filename, pattern=None):
    ...
    def unzip_to_dir(filename, dirname):
        ...
        p.communicate()
        if p.returncode != 0:
            raise ExternalError('failed to unzip input target-files "%s"' % (filename,))
...
def GetKeyPasswords(keylist):
    ...
    for k in sorted(keylist):
        ...
        p = Run(['openssl', 'pkcs8', '-in', k + OPTIONS.private_key_suffix, '-inform', 'DER', '-nocrypt'], stdin=devnull.fileno(), stdout=devnull.fileno(), stderr=subprocess.STDOUT)
        p.communicate()
        if p.returncode == 0:
            # Definitely an unencrypted key.
            no_passwords.append(k)
        else:
            p = Run(['openssl', 'pkcs8', '-in', k + OPTIONS.private_key_suffix, '-inform', 'DER', '-passin', 'pass:'], stdin=devnull.fileno(), stdout=devnull.fileno(), stderr=subprocess.PIPE)
...
def GetMinSdkVersion(apk_name):
    ...
    if err:
        raise ExternalError('Failed to obtain minSdkVersion: aapt return code %s' % (p.returncode,))
...
def GetMinSdkVersionInt(apk_name, codename_to_api_level_map):
    ...
    try:
        ...
        if version in codename_to_api_level_map:
            ...
            raise ExternalError("Unknown minSdkVersion: '%s'. Known codenames: %s" % (version, codename_to_api_level_map))


def SignFile(input_name, output_name, key, password, min_api_level=None, codename_to_api_level_map=dict(), whole_file=False):
    ...
    cmd = [OPTIONS.java_path, OPTIONS.java_args, '-Djava.library.path=' + java_library_path, '-jar', os.path.join(OPTIONS.search_path, OPTIONS.signapk_path)]
    ...
    cmd.extend([key + OPTIONS.public_key_suffix, key + OPTIONS.private_key_suffix, input_name, output_name])
...
def ReadApkCerts(tf_zip):
    ...
    for line in tf_zip.read('META/apkcerts.txt').split('\n'):
        ...
        if not line:
            continue
        m = re.match(r'^name="(.*)"\s+certificate="(.*)"\s+' r'private_key="(.*)"$', line)
        if m:
            ...
            if cert in SPECIAL_CERT_STRINGS and not privkey:
                certmap[name] = cert
            elif cert.endswith(OPTIONS.public_key_suffix) and privkey.endswith(OPTIONS.private_key_suffix) and cert[:-public_key_suffix_len] == privkey[:-private_key_suffix_len]:
...
def ParseOptions(argv, docstring, extra_opts='', extra_long_opts=(), extra_option_handler=None):
    ...
    try:
        opts, args = getopt.getopt(
            argv,
            'hvp:s:x:' + extra_opts,
            ['help', 'verbose', 'path=', 'signapk_path=', 'signapk_shared_library_path=', 'extra_signapk_args=', 'java_path=', 'java_args=', 'public_key_suffix=', 'private_key_suffix=', 'boot_signer_path=', 'boot_signer_args=', 'verity_signer_path=', 'verity_signer_args=', 'device_specific=', 'extra=']
        ...
        try:
            ...
            for d in diff_iter:
                ...
                if patch is None:
                    ...
                    print('%8.2f sec %8d / %8d bytes (%6.2f%%) %s' % (dur, len(patch), tf.size, 100.0 * len(patch) / tf.size, name))
            lock.release()
...
class BlockDifference(object):
    def __init__(self, partition, tgt, src=None, check_first_block=False, version=None, disable_imgdiff=False):
        self.tgt = tgt
        self.src = src
        ...
        if version is None:
            ...
            if OPTIONS.info_dict:
                version = max(int(i) for i in OPTIONS.info_dict.get('blockimgdiff_versions', '1').split(','))
        self.version = version

        b = blockimgdiff.BlockImageDiff(tgt, src, threads=OPTIONS.worker_threads, version=self.version, disable_imgdiff=self.disable_imgdiff)
    ...
    def WriteStrictVerifyScript(self, script):
        ...
        script.AppendExtra('range_sha1("%s", "%s") == "%s" && ' 'ui_print("    Verified.") || ' 'ui_print("\\"%s\\" has unexpected contents.");' % (self.device, ranges_str, self.tgt.TotalSha1(include_clobbered_blocks=True), self.device))
        script.AppendExtra('')
    ...
    def WriteVerifyScript(self, script, touched_blocks_only=False):
        ...
        # incremental OTA
            ...
            ranges_str = ranges.to_string_raw()
            if self.version >= 4:
                script.AppendExtra(('if (range_sha1("%s", "%s") == "%s" || ' 'block_image_verify("%s", ' 'package_extract_file("%s.transfer.list"), ' '"%s.new.dat", "%s.patch.dat")) then') % (self.device, ranges_str, expected_sha1, self.device, partition, partition, partition))
            elif self.version == 3:
                script.AppendExtra(('if (range_sha1("%s", "%s") == "%s" || ' 'block_image_verify("%s", ' 'package_extract_file("%s.transfer.list"), ' '"%s.new.dat", "%s.patch.dat")) then') % (self.device, ranges_str, expected_sha1, self.device, partition, partition, partition))
            else:
                script.AppendExtra('if range_sha1("%s", "%s") == "%s" then' % (self.device, ranges_str, self.src.TotalSha1()))
            ...
            if self.version >= 4:
                ...
                script.AppendExtra(
                    (
                        ...
                        'endif;'
                    ).format(device=self.device, ranges=ranges_str, partition=partition, code=code)
                )
    ...
    def _WritePostInstallVerifyScript(self, script):
        ...
        ranges = self.tgt.care_map
        ranges_str = ranges.to_string_raw()
        script.AppendExtra('if range_sha1("%s", "%s") == "%s" then' % (self.device, ranges_str, self.tgt.TotalSha1(include_clobbered_blocks=True)))

        # Bug: 20881595
        # Verify that extended blocks are really zeroed out.
        if self.tgt.extended:
            ranges_str = self.tgt.extended.to_string_raw()
            script.AppendExtra('if range_sha1("%s", "%s") == "%s" then' % (self.device, ranges_str, self._HashZeroBlocks(self.tgt.extended.size())))
    ...
    def _WriteUpdate(self, script, output_zip):
        ZipWrite(output_zip, '{}.transfer.list'.format(self.path), '{}.transfer.list'.format(self.partition))
        ZipWrite(output_zip, '{}.new.dat'.format(self.path), '{}.new.dat'.format(self.partition))
        ZipWrite(output_zip, '{}.patch.dat'.format(self.path), '{}.patch.dat'.format(self.partition), compress_type=zipfile.ZIP_STORED)
        ...
        call = 'block_image_update("{device}", ' 'package_extract_file("{partition}.transfer.list"), ' '"{partition}.new.dat", "{partition}.patch.dat") ||\n' '  abort("E{code}: Failed to update {partition} image.");'.format(device=self.device, partition=self.partition, code=code)
...
PARTITION_TYPES = {'yaffs2': 'MTD', 'mtd': 'MTD', 'ext4': 'EMMC', 'emmc': 'EMMC', 'f2fs': 'EMMC', 'squashfs': 'EMMC'}
...
""" % {'type': recovery_type, 'device': recovery_device, 'sha1': recovery_img.sha1, 'size': recovery_img.size}
    else:
        sh = """#!/system/bin/sh
...
""" % {'boot_size': boot_img.size, 'boot_sha1': boot_img.sha1, 'recovery_size': recovery_img.size, 'recovery_sha1': recovery_img.sha1, 'boot_type': boot_type, 'boot_device': boot_device, 'recovery_type': recovery_type, 'recovery_device': recovery_device, 'bonus_args': bonus_args}
    ...
    for init_rc_file in init_rc_files:
        if not init_rc_file.startswith('init.') or not init_rc_file.endswith('.rc'):
            continue
